...
@Component({
  ...
  <div class="min-h-screen bg-gray-50 p-6 lg:p-8">
    <div class="mb-8">
      <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight">Admin Dashboard</h1>
      <p class="text-sm text-gray-500 mt-0.5">Overview of the Tbibi platform</p>
    </div>

    <!-- Error/Loading state -->
    <div *ngIf="loading" class="text-gray-500 mb-6">Loading statistics...</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8" *ngIf="stats">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow">
        ...
        <p class="text-sm font-medium text-gray-500 mt-1">Active Professionals</p>
      </div>
      ...
      <div class="bg-white rounded-2xl border-2 border-orange-100 p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
        ...
        <p class="relative z-10 text-sm font-medium text-orange-700 mt-1">Pending Accounts</p>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow">
        ...
        <p class="text-sm font-medium text-gray-500 mt-1">Blocked Accounts</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6">
      ...
      <div class="flex flex-wrap gap-4">
        <a routerLink="/admin/approvals" class="px-5 py-3 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors flex items-center gap-2">
          Manage Approvals <span class="bg-white/20 px-2 py-0.5 rounded-lg text-xs">{{stats?.pendingApprovals || 0}}</span>
        </a>
