...
@Component({
  ...
  template: `
    ...
    <div class="p-8" *ngIf="medicine && !loading">
      <button (click)="goBack()"
              class="mb-6 flex items-center gap-2 text-gray-600 hover:text-blue-600 transition-colors font-medium">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="grid grid-cols-1 md:grid-cols-2">
          ...
          <div class="bg-gray-50 flex items-center justify-center p-12 min-h-[400px]">
            <img *ngIf="medicine.imageUrl" [src]="medicine.imageUrl" [alt]="medicine.medicineName"
                 class="max-w-full max-h-[400px] object-contain drop-shadow-lg" />
            <span *ngIf="!medicine.imageUrl" class="text-9xl">💊</span>
          </div>
            ...
            <div class="mb-auto">
              <div class="flex items-center gap-3 mb-4">
                ...
                <span *ngIf="medicine.requiresPrescription"
                      class="px-3 py-1 text-sm bg-red-100 text-red-700 rounded-full font-medium">
              ...
              <div class="space-y-2 text-gray-600 mb-8">
                ...
                <p *ngIf="medicine.description">{{medicine.description}}</p>
              </div>

              <div class="text-3xl font-bold text-blue-600 mb-8">
                {{medicine.price.toFixed(2)}} DT
              </div>
            </div>

            <div class="flex items-center gap-6 pt-8 border-t border-gray-100" *ngIf="medicine.stock > 0">
              <div class="flex items-center border border-gray-300 rounded-xl overflow-hidden">
                <button (click)="qty = qty > 1 ? qty - 1 : 1"
                        class="w-12 h-12 flex items-center justify-center bg-gray-50 hover:bg-gray-100 text-gray-600 text-lg transition-colors">−</button>
                <div class="w-16 h-12 flex items-center justify-center font-bold text-lg border-x border-gray-300">{{qty}}</div>
                <button (click)="qty = qty < medicine.stock ? qty + 1 : medicine.stock"
                        class="w-12 h-12 flex items-center justify-center bg-gray-50 hover:bg-gray-100 text-gray-600 text-lg transition-colors">+</button>
              </div>

              <button (click)="addToCart()"
                      class="flex-1 h-14 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-semibold text-lg shadow-lg shadow-blue-200">
                Add to Cart
              </button>
            </div>
  ...
  addToCart() {
    if (this.medicine) {
      ...
      MainLayoutComponent.showToast(
        `${this.qty}x ${this.medicine.medicineName} added to cart`,
        'success'
      );
    }
  }
