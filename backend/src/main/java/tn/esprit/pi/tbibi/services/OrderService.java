...
public class OrderService implements IOrderService {
        ...
        for (OrderLine line : orderLines) {
            ...
            if (med.isPrescriptionRequired()) {
                ...
                for (Prescription p : patientPrescriptions) {
                    if (p.getStatus() != PrescriptionStatus.VALIDATED && p.getStatus() != PrescriptionStatus.PENDING) continue;
    ...
    @Override
    @Transactional
    public List<tn.esprit.pi.tbibi.DTO.order.PatientSpendingAnalyticsDTO> getPatientSpendingAnalytics(Integer patientId) {
        return orderRepo.getPatientSpendingAnalytics(patientId);
    }

    @Override
    @Transactional
    public List<tn.esprit.pi.tbibi.DTO.order.MedicinePurchaseHistoryDTO> getMedicinePurchaseHistory(Integer userId, String medicineName) {
    ...
    private boolean isSimilarityMatch(String prescribed, String pharmacy) {
        if (prescribed == null || pharmacy == null) return false;
        prescribed = prescribed.trim();
        pharmacy = pharmacy.trim();
        
        // Direct or partial match
        if (prescribed.contains(pharmacy) || pharmacy.contains(prescribed)) return true;
