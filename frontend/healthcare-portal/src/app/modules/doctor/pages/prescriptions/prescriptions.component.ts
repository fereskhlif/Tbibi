  ...
  <div *ngIf="showModal"
    ...
    <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
      ...
      <div class="p-6 space-y-5 overflow-y-auto flex-1">
          ...
          <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-2">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <select [(ngModel)]="form.typeCategory" (ngModelChange)="onTypeChange($event)"
                  class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 transition bg-white">
              ...
              <!-- Analysis Subtype (shown only when ANALYSE is selected) -->
              <div *ngIf="form.typeCategory === 'ANALYSE'">
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Type d'analyse *</label>
                <select [(ngModel)]="form.analysisSubType" (ngModelChange)="onAnalysisSubTypeChange($event)"
                  class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 transition bg-white">
                  <option value="" disabled>Sélectionner le type d'analyse...</option>
                  <option value="ANALYSE_DIAGNOSTIQUE">🔬 Analyses médicales diagnostiques</option>
                  <option value="ANALYSE_MICROBIOLOGIQUE">🦠 Analyses microbiologiques</option>
                  <option value="EXAMEN_ANATOMOPATHOLOGIQUE">🧬 Examens anatomopathologiques</option>
                  <option value="TEST_GENETIQUE">🧪 Tests génétiques</option>
                </select>
              </div>
        ...
        <!-- Prescription Date -->
        <div>
          <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Date (Acte & Prescription)</label>
...
export class DoctorPrescriptionsComponent implements OnInit, OnDestroy {
  ...
  onTypeChange(value: string): void {
    ...
    // For non-analysis types, typeOfActe equals the category directly
    if (value !== 'ANALYSE') {
  ...
  onAnalysisSubTypeChange(value: string): void {
    ...
    this.form.typeOfActe = value; // store the specific analysis subtype
  }
